...
async function generateSiteJSON(options, requestHost, sub, pwd) {
  ...
  let valid_files = files.filter(
    (file) => file.endsWith(".js") && !file.startsWith("_")
  ) // 筛选出不是 "_" 开头的 .js 文件
  let sort_list = []
  if (sub) {
    if (sub.mode === 0) {
      valid_files = valid_files.filter((it) =>
        new RegExp(sub.reg || ".*").test(it)
      )
    } else if (sub.mode === 1) {
      valid_files = valid_files.filter(
        (it) => !new RegExp(sub.reg || ".*").test(it)
      )
    }
    let sort_file = path.join(path.dirname(subFilePath), `./order_common.html`)
    if (!existsSync(sort_file)) {
      sort_file = path.join(
        path.dirname(subFilePath),
        `./order_common.example.html`
      )
    }
    if (sub.sort) {
    ...
    return {
      func: async ({
        ...
        if (baseName === "push_agent") {
          ...
          SitesMap.hasOwnProperty(baseName) &&
          Array.isArray(SitesMap[baseName])
        ) {
          SitesMap[baseName].forEach((it) => {
            let key = `drpyS_${it.alias}`
            let name = `${it.alias}(DS)`
            let ext =
              it.queryObject.type === "url"
                ? it.queryObject.params
                : it.queryStr
            if (ext) {
  ...
  if (ENV.get("enable_dr2", "1") === "1") {
    ...
    let dr2_valid_files = dr2_files.filter(
      (file) =>
        (file.endsWith(".js") || file.endsWith(".py")) && !file.startsWith("_")
    ) // 筛选出不是 "_" 开头的 .js 文件
    // log(dr2_valid_files);
    log(
      `开始生成dr2的t3配置，dr2Dir:${dr2Dir},源数量: ${dr2_valid_files.length}`
    )
      ...
      return {
        func: async ({ file, dr2Dir, requestHost, pwd, drpy, SitesMap }) => {
          ...
          const baseName = isJs
            ...
            : path.basename(file, ".py") // 去掉文件扩展名
          let api = isJs
            ? `${requestHost}/libs_dr2/drpy2.min.js` // 使用内置drpy2
            : `${requestHost}/js/${file}`
          ...
          if (baseName === "push_agent") {
            ...
            let key = isJs ? `drpy2_${baseName}` : `${baseName}`
            let name = isJs ? `${baseName}(DR2)` : `${baseName}(T3)`
            fileSites.push({ key, name, ext })
          }
  ...
  if (ENV.get("enable_link_data", "0") === "1") {
    ...
    try {
      let link_data = readFileSync(
        path.join(rootDir, "./data/settings/link_data.json"),
        "utf-8"
      )
...
async function generateParseJSON(jxDir, requestHost) {
    ...
    return {
      func: async ({ file, jxDir, requestHost, drpy }) => {
        ...
        try {
          ...
          throw new Error(
            `Error parsing jx object for file: ${file}, ${e.message}`
          )
        }
  ...
  await batchExecute(tasks, listener)
  let sorted_parses = naturalSort(parses, "name", [
    "JSON并发",
    "JSON合集",
    "虾米",
    "奇奇",
...
export default (fastify, options, done) => {
  ...
  fastify.get(
    "/config*",
    { preHandler: [validatePwd, validateBasicAuth] },
    async (request, reply) => {
      ...
      try {
        ...
        const protocol =
          ...
          (request.socket.encrypted ? "https" : "http") // http 或 https
        ...
        let not_local =
          cfg_path.startsWith("/1") || cfg_path.startsWith("/index")
        let requestHost = not_local
          ? `${protocol}://${hostname}`
          : `http://127.0.0.1:${options.PORT}` // 动态生成根地址
        let requestUrl = not_local
          ? `${protocol}://${hostname}${request.url}`
          : `http://127.0.0.1:${options.PORT}${request.url}` // 动态生成请求链接
        ...
        //     }
        // }
        const getFilePath = (cfgPath, rootDir, fileName) =>
          path.join(rootDir, `data/cat/${fileName}`)
        const processContent = (content, cfgPath, requestUrl) =>
          content.replace(
            "$config_url",
            requestUrl.replace(
              cfgPath,
              `/1?sub=all&pwd=${process.env.API_PWD || ""}`
            )
          )
        ...
        const configObj = {
          sites_count: siteJSON.sites.length,
          ...playerJSON,
          ...siteJSON,
          ...parseJSON,
          ...livesJSON,
        ...
        reply.status(500).send({
          error: "Failed to generate site JSON",
