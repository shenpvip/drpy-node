...
function generateSiteJSON() {
  ...
  const sites = files
    ...
    .map((file) => {
      ...
      const api = `https://drpy.900730.xyz/api/${baseName}`
      return {
        key,
        name,
        type: 4, // 固定值
...
fastify.get("/config", async (request, reply) => {
  try {
    ...
    reply
      .status(500)
      .send({ error: "Failed to generate site JSON", details: error.message })
  }
})
...
fastify.get("/index", async (request, reply) => {
  try {
    ...
    reply
      .status(500)
      .send({ error: "Failed to read index.json", details: error.message })
  }
})
...
fastify.get("/api/:module", async (request, reply) => {
  ...
  try {
    ...
    reply.status(500).send({
      error: `Failed to process request for module ${moduleName}: ${error.message}`,
    })
  }
})
